...
def main():
 p=argparse.ArgumentParser(description=__doc__);p.add_argument('--prefix',type=Path,default=ROOT/'.cache/aqueous-master');p.add_argument('--output',type=Path,default=ROOT/'artifacts/aqueous-master/upstream');a=p.parse_args();a.prefix=a.prefix.resolve();a.output=a.output.resolve();a.output.mkdir(parents=True,exist_ok=True)
 source=a.prefix/'source';meta=json.loads((a.prefix/'metadata.json').read_text());assert meta['revision']=='1d038dc3bafa0044d9599f8f51f84105a6a85bb3'
 ...
 try:
  ...
  replacements={"helper=ROOT.parent/'settingsApplication/zig-out/bin/aqueous-config'":'helper=Path('+repr(str(a.prefix/'bin/aqueous-config'))+')',"str(ROOT/'.deps/wlroots-render-hook/lib')":repr(str(Path(meta['patched_wlroots_pkgconfig']).parent)),"source_revision=subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip()":'source_revision='+repr(meta['revision'])}
  ...
  with (a.output/'preview.log').open('w') as log:subprocess.run([a.prefix/"test-venv/bin/python",harness],env=dict(env,AQUEOUS_COMPOSITOR_BIN=str(instrumented/'bin/aqueous')),stdout=log,stderr=subprocess.STDOUT,check=True,timeout=300)
