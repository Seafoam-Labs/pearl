...
if __name__=='__main__':
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--evidence',type=Path,default=ROOT/'artifacts/aqueous-master');a=p.parse_args();a.evidence=a.evidence.resolve();result=evaluate(a.evidence);a.evidence.mkdir(parents=True,exist_ok=True);(a.evidence/'gate.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result,indent=2));raise SystemExit(not result['release_ready'])
