...
TARGETS=('integration','test-components','test-adapter','test-surfaces','test-desktop','test-services','test-connectivity','test-session-services','test-preferences','test-aqueous-settings','test-security','test-lock','test-clipboard-capture','test-dock-islands','test-release')
def main():
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--output',type=Path,default=ROOT/'artifacts/t16/functional');p.add_argument('--resume',action='store_true',help='Retain prior target results and log failed attempts when rerunning selected targets');p.add_argument('--targets',nargs='+',choices=TARGETS,default=TARGETS);a=p.parse_args()
    ...
    try:
        ...
        report['status']='passed' if all(v['exit_code']==0 for v in report['targets'].values()) else 'failed'
