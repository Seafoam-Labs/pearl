...
p=argparse.ArgumentParser();p.add_argument('--focus');p.add_argument('--action');a=p.parse_args()
...
while queue and seen<5000:
 ...
 try:
  ...
  if showing and name:nodes.append(dict(name=name[:512],role=role,focused=states.contains(Atspi.StateType.FOCUSED)))
