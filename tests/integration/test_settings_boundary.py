    ...
    try:
        with PrivateSession(args.output / 'first') as first, PrivateSession(args.output / 'second') as second:
            ...
            with Frontend(path, token, display) as f, Frontend(path2, token2, display2) as g:
                ...
                assert all(state['capabilities'][k] for k in ('handshake', 'committed_appearance', 'page_snapshots', 'pearl_draft', 'aqueous_draft', 'live_controls', 'prompts', 'display_preview', 'community_themes', 'theme_assets', 'application_profiles', 'notification_filters', 'bar_widgets', 'launcher_icon', 'night_light'))
                assert not any(v for k, v in state['capabilities'].items() if k not in ('handshake', 'committed_appearance', 'page_snapshots', 'pearl_draft', 'aqueous_draft', 'live_controls', 'prompts', 'display_preview', 'community_themes', 'theme_assets', 'application_profiles', 'notification_filters', 'bar_widgets', 'launcher_icon', 'night_light'))
