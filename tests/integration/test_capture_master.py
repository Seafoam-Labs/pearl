...
def main():
 ...
 p.add_argument('--prefix',type=Path,default=ROOT/'.cache/aqueous-master');p.add_argument('--output',type=Path,default=ROOT/'artifacts/aqueous-master/capture');args=p.parse_args();args.pearl=args.pearl.resolve();args.ctl=args.ctl.resolve();args.prefix=args.prefix.resolve();args.output=args.output.resolve();args.output.mkdir(parents=True,exist_ok=True)
 ...
 try:
  with PrivateSession(args.output/'session',tool_prefix=args.prefix) as s:
   ...
   fixture=s.child('source',['python3',ROOT/'tests/fixtures/desktop/application.py','--id','org.pearl.CaptureSource','--title','Capture source']);fixture.expect('event=fixture-ready')
   time.sleep(.4)
   ...
   overlap=s.child('overlap',['python3',ROOT/'tests/fixtures/desktop/application.py','--id','org.pearl.CaptureOverlap','--title','Unrelated overlap']);overlap.expect('event=fixture-ready')
   ...
   # Pinned wlroots reports unavailable metadata for scene capture. No SDR guess.
   assert 'color metadata unavailable' in v['message'] and not v['ready'],v
   checks['native-isolated-source-withholds-undescribed-png']=True
   ...
   overlap.stop();ctl(s,args.ctl,'quit');app.proc.wait(timeout=10);clean(app)
   log='\n'.join(app.lines)
