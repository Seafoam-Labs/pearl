...
fn sessionChanged(data: ?*anyopaque) callconv(.c) c_int {
    ...
    if (client.availability == .ready and self.control_server == null) {
        ...
        self.settings_server.?.backend = .{ .service = &self.surfaces.?.preferences, .aqueous = &self.surfaces.?.aqueous_settings, .context = self, .allowed = settingsAllowed, .live = .{ .night_light = &self.surfaces.?.night_light, .plugins = self.surfaces.?.plugins, .audio = &self.surfaces.?.audio, .network = &self.surfaces.?.network, .bluetooth = &self.surfaces.?.bluetooth, .power = &self.surfaces.?.power, .session = &self.surfaces.?.session_services, .lifecycle = &self.surfaces.?.lifecycle, .layout = &self.surfaces.?.layout.?, .layout_context = &self.surfaces.?, .layout_rows = @import("../ui/surfaces/manager.zig").Manager.settingsLayoutRows, .launcher_icon_retry = @import("../ui/surfaces/manager.zig").Manager.settingsLauncherIconRetry, .layout_action = @import("../ui/surfaces/manager.zig").Manager.settingsLayoutAction } };
