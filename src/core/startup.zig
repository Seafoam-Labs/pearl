...
pub fn validate(mode: Mode, environment: Environment) !void {
    ...
    while (desktops.next()) |desktop| {
        for ([_][]const u8{ "Aqueous", "Aqueous-Git", "Aqueous-Intel-Git" }) |name| {
            if (std.ascii.eqlIgnoreCase(desktop, name)) supported = true;
        }
    }
    ...
    const prefix = for ([_][]const u8{ "/aqueous/", "/aqueous-git/", "/aqueous-intel-git/" }) |candidate| {
        if (std.mem.startsWith(u8, suffix, candidate)) break candidate;
    } else return error.InvalidAqueousEndpoint;
    if (!std.mem.endsWith(u8, suffix, "/ipc.sock") or suffix.len <= prefix.len + "/ipc.sock".len)
        return error.InvalidAqueousEndpoint;
    const instance = suffix[prefix.len .. suffix.len - "/ipc.sock".len];
...
pub fn diagnostic(err: anyerror) []const u8 {
    return switch (err) {
        ...
        error.InvalidAqueousEndpoint => "AQUEOUS_SOCKET must identify this runtime's aqueous, aqueous-git or aqueous-intel-git session IPC socket. Launch Pearl from Aqueous or use --demo.",
...
test "Git desktop sessions accept private endpoints and reject lookalikes" {
    var env: Environment = .{ .desktop = "Aqueous-Git", .runtime = "/run/user/1000", .display = "wayland-1", .endpoint = "/run/user/1000/aqueous-git/abc/ipc.sock" };
    try validate(.session, env);
    env.desktop = "wlroots:aqueous-intel-git";
    env.endpoint = "/run/user/1000/aqueous-intel-git/abc/ipc.sock";
    try validate(.session, env);
    env.desktop = "Aqueous-Git-Other";
    try std.testing.expectError(error.AqueousSessionRequired, validate(.session, env));
    env.desktop = "Aqueous-Git";
    for ([_][]const u8{ "/run/user/10001/aqueous-git/abc/ipc.sock", "/run/user/1000/aqueous-git-other/abc/ipc.sock", "/run/user/1000/aqueous-git/../ipc.sock", "/run/user/1000/aqueous-git//ipc.sock", "/run/user/1000/aqueous-intel-git/a/b/ipc.sock" }) |endpoint| {
        env.endpoint = endpoint;
        try std.testing.expectError(error.InvalidAqueousEndpoint, validate(.session, env));
    }
}
