...
pub fn digest(bytes: []const u8) [64]u8 {
    var raw: [32]u8 = undefined; std.crypto.hash.sha2.Sha256.hash(bytes, &raw, .{}); return std.fmt.bytesToHex(raw, .lower);
}
fn etag(alloc: std.mem.Allocator, path: [:0]const u8) ![:0]const u8 {
    const file = gio.File.newForPath(path); defer file.unref();
...
pub fn replace(path: [:0]const u8, bytes: []const u8, expected: Read, cancel: ?*gio.Cancellable) !void {
    ...
    if (err) |e| { defer e.free(); if (e.matches(gio.ioErrorQuark(), @intFromEnum(gio.IOErrorEnum.wrong_etag)) != 0) return error.Conflict; }
    if (ok == 0) return error.SaveFailed;
}
pub fn atomic(path: [:0]const u8, bytes: []const u8, exclusive: bool) !void {
    const temp = try std.fmt.allocPrintSentinel(a, "{s}.XXXXXX", .{path}, 0); defer a.free(temp);
    ...
    var offset: usize = 0;
    while (offset < bytes.len) { const n = std.c.write(fd, bytes[offset..].ptr, bytes.len-offset); if (n <= 0) return error.SaveFailed; offset += @intCast(n); }
    ...
    const parent = try a.dupeZ(u8, std.fs.path.dirname(path) orelse "/"); defer a.free(parent);
    const dir = std.c.open(parent, .{ .ACCMODE = .RDONLY, .DIRECTORY = true, .CLOEXEC = true }, @as(c_uint, 0));
    if (dir >= 0) { defer _ = std.c.close(dir); if (std.c.fsync(dir) != 0) return error.SaveFailed; }
}
