...
pub const Record = struct {
    id: u32 = 0, serial: u64 = 0, active: bool = false, transient: bool = false, resident: bool = false,
    owner: Text(256) = .{}, app: Text(160) = .{}, summary: Text(256) = .{}, body: Text(2048) = .{}, icon: Text(160) = .{},
    actions: [8]Action = @splat(.{}), action_count: usize = 0,
    urgency: u8 = 1, deadline: i64 = 0, toast_until: i64 = 0,
};
pub const Model = struct {
    records: [64]Record = @splat(.{}), next_id: u32 = 0, serial: u64 = 0, dnd: bool = false, locked: bool = false,
    ...
    pub fn add(self: *Model, input: Record, replaces: u32, timeout: i32, now: i64) !*Record {
        ...
        slot.?.* = input;
        const r = slot.?;
        r.id = id; r.serial = self.serial; r.active = true;
    ...
    pub fn close(self: *Model, id: u32) bool {
        ...
        r.active = false; r.deadline = 0; r.toast_until = 0;
        if (r.transient) r.* = .{};
        self.serial += 1;
        return true;
    }
    ...
    pub fn clearHistory(self: *Model) void {
        for (&self.records) |*r| if (!r.active) { r.* = .{}; };
        self.serial += 1;
    }
};
...
pub fn sanitize(comptime n: usize, value: []const u8) Text(n) {
    ...
    var i: usize = 0;
    while (i < value.len) {
        const size = std.unicode.utf8ByteSequenceLength(value[i]) catch { i += 1; continue; };
        if (i + size > value.len) break;
        const cp = std.unicode.utf8Decode(value[i..][0..size]) catch { i += 1; continue; };
        ...
        if (allowed) {
            ...
            @memcpy(out.bytes[out.len..][0..size], value[i..][0..size]); out.len += size;
        }
        i += size;
    }
    return out;
