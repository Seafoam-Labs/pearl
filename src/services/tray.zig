...
pub const Tray = struct {
    ...
    fn acquired(_: *gio.DBusConnection, _: [*:0]const u8, data: ?*anyopaque) callconv(.c) void {
        ...
        self.external = .{};
        self.external_epoch += 1;
        self.bus.emit(null, watcher_path, watcher_name, "StatusNotifierHostRegistered", db.tuple(&.{}));
        self.notify();
    }
    ...
    fn hostDone(data: *anyopaque, token: u64, value: ?*glib.Variant) void {
        ...
        self.err = if (value == null) "Could not register with the existing tray watcher." else null;
        self.notify();
    }
    fn watcherProperties(data: *anyopaque, token: u64, value: ?*glib.Variant) void {
        ...
        defer props.unref();
        const entries = db.lookup(props, "RegisteredStatusNotifierItems", "as") orelse return;
        defer entries.unref();
    ...
    fn register(self: *Tray, registration: []const u8, sender: ?[]const u8) !void {
        ...
        var bus_name: Text(256) = .{};
        var object_path: Text(512) = .{};
        if (registration[0] == '/') {
            bus_name.set(sender orelse return error.InvalidValue);
            object_path.set(registration);
        } else {
            ...
            bus_name.set(registration[0..split]);
            object_path.set(if (split == registration.len) "/StatusNotifierItem" else registration[split..]);
        }
        ...
        for (&self.items) |*item| if (item.name.len == 0) {
            ...
            item.registration.set(std.fmt.bufPrint(&buf, "{s}{s}", .{ bus_name.slice(), object_path.slice() }) catch unreachable);
            transport.resolve(self.bus, self, item.generation, bus_name.z(), resolvedItem) catch |err| {
                item.* = .{};
                return err;
            };
            return;
    ...
    fn resolvedItem(data: *anyopaque, token: u64, value: ?*glib.Variant) void {
        ...
        item.owner = transport.childText(256, v, 0);
        item.dirty = true;
        self.arm();
    }
    ...
    pub fn signal(self: *Tray, sender: []const u8, object_path: []const u8, interface: []const u8, member: []const u8, params: *glib.Variant) void {
        ...
        for (&self.items) |*item| if (item.owner.len != 0 and std.mem.eql(u8, item.owner.slice(), sender)) {
            if (std.mem.eql(u8, item.path.slice(), object_path) and (std.mem.eql(u8, interface, item_iface) or std.mem.eql(u8, interface, "org.freedesktop.DBus.Properties"))) {
                item.dirty = true;
                self.arm();
            }
            if (std.mem.eql(u8, item.menu.slice(), object_path) and std.mem.eql(u8, interface, menu_iface) and (std.mem.eql(u8, member, "LayoutUpdated") or std.mem.eql(u8, member, "ItemsPropertiesUpdated"))) {
                item.menu_ready = false;
                item.menu_revision += 1;
                item.menu_dirty = true;
                self.arm();
                self.notify();
            }
        };
    }
    fn arm(self: *Tray) void {
    ...
    fn tick(data: ?*anyopaque) callconv(.c) c_int {
        ...
        for (&self.items) |*item| {
            if (item.dirty and !item.loading and item.owner.len != 0) {
                item.dirty = false;
                item.loading = true;
                transport.getAll(self.bus, self, item.generation, item.owner.z(), item.path.z(), item_iface, properties) catch {
                    item.loading = false;
                };
            }
    ...
    fn properties(data: *anyopaque, token: u64, value: ?*glib.Variant) void {
        ...
        for (item.icon.slice()) |c| if (!(std.ascii.isAlphanumeric(c) or c == '-' or c == '_' or c == '.')) {
            item.icon = .{};
            break;
        };
        ...
        item.menu = db.string(props, "Menu", "o");
        item.is_menu = db.boolean(props, "ItemIsMenu");
        if (!std.mem.eql(u8, old_menu.slice(), item.menu.slice())) {
            item.node_count = 0;
            item.menu_token = 0;
            item.menu_loading = false;
            item.menu_ready = false;
            item.menu_revision += 1;
        }
        if (item.image) |image| image.unref();
        item.image = null;
        if (db.lookup(props, if (std.mem.eql(u8, item.status.slice(), "NeedsAttention")) "AttentionIconPixmap" else "IconPixmap", "a(iiay)")) |images| {
            defer images.unref();
            item.image = decodePixmap(images);
        }
        if (item.image == null) if (db.lookup(props, "IconPixmap", "a(iiay)")) |images| {
            defer images.unref();
            item.image = decodePixmap(images);
        };
    ...
    pub fn contextMenu(self: *Tray, generation: u64) !void {
        const item = self.find(generation) orelse return error.InvalidValue;
        try self.bus.call(self, generation, item.owner.z(), item.path.z(), item_iface, "ContextMenu", db.tuple(&.{ glib.Variant.newInt32(0), glib.Variant.newInt32(0) }), "()", actionDone);
    }
    pub fn scroll(self: *Tray, generation: u64, delta: i32, vertical: bool) !void {
        const item = self.find(generation) orelse return error.InvalidValue;
    ...
    fn fetchMenu(self: *Tray, item: *Item) void {
        item.menu_loading = true;
        self.menu_sequence += 1;
        item.menu_token = self.menu_sequence;
        item.menu_dirty = false;
        self.bus.call(self, item.menu_token, item.owner.z(), item.menu.z(), menu_iface, "GetLayout", db.tuple(&.{ glib.Variant.newInt32(0), glib.Variant.newInt32(8), db.array("s", &.{}) }), "(u(ia{sv}av))", menuDone) catch {
            item.menu_loading = false;
        };
    }
    fn menuDone(data: *anyopaque, token: u64, value: ?*glib.Variant) void {
        const self: *Tray = @ptrCast(@alignCast(data));
        const item = blk: {
            for (&self.items) |*entry| if (entry.name.len > 0 and entry.menu_token == token) break :blk entry;
            return;
        };
        item.menu_loading = false;
        item.menu_error = true;
        item.menu_ready = false;
        item.node_count = 0;
        item.menu_revision += 1;
        if (value) |v| if (v.getSize() <= 256 * 1024) {
            const layout = v.getChildValue(1);
            defer layout.unref();
            parseNode(item, layout, -1, 0) catch {
                item.node_count = 0;
                self.notify();
                return;
            };
    ...
    fn actionDone(data: *anyopaque, token: u64, value: ?*glib.Variant) void {
        const self: *Tray = @ptrCast(@alignCast(data));
        if (self.find(token) == null) return;
        self.err = if (value == null) "The tray item did not accept the action." else null;
        self.notify();
    }
    ...
    fn method(_: *gio.DBusConnection, sender: ?[*:0]const u8, _: [*:0]const u8, _: ?[*:0]const u8, member: [*:0]const u8, params: *glib.Variant, invocation: *gio.DBusMethodInvocation, data: ?*anyopaque) callconv(.c) void {
        const self: *Tray = @ptrCast(@alignCast(data.?));
        if (!self.watcher) {
            invocation.returnDbusError("org.freedesktop.DBus.Error.NotSupported", "Another watcher owns the name.");
            return;
        }
        if (params.getSize() > 1024) {
            invocation.returnDbusError("org.freedesktop.DBus.Error.LimitsExceeded", "Registration too long.");
            return;
        }
        const service = transport.childText(768, params, 0);
        if (std.mem.eql(u8, std.mem.span(member), "RegisterStatusNotifierItem")) {
            self.register(service.slice(), if (sender) |s| std.mem.span(s) else null) catch {
                invocation.returnDbusError("org.freedesktop.DBus.Error.InvalidArgs", "Invalid registration or item limit reached.");
                return;
            };
        } else if (std.mem.eql(u8, std.mem.span(member), "RegisterStatusNotifierHost")) {
            if (gio.dbusIsName(service.z()) == 0) {
                invocation.returnDbusError("org.freedesktop.DBus.Error.InvalidArgs", "Invalid host name.");
                return;
            }
        } else {
    ...
    fn property(_: *gio.DBusConnection, _: ?[*:0]const u8, _: [*:0]const u8, _: [*:0]const u8, prop: [*:0]const u8, _: **glib.Error, data: ?*anyopaque) callconv(.c) ?*glib.Variant {
        ...
        if (std.mem.eql(u8, key, "RegisteredStatusNotifierItems")) {
            var values: [32]*glib.Variant = undefined;
            var n: usize = 0;
            for (&self.items) |*item| if (item.ready) {
                values[n] = db.str(item.registration.z());
                n += 1;
            };
...
fn optionalBool(props: *glib.Variant, key: [:0]const u8, default: bool) bool {
    const v = db.lookup(props, key, "b") orelse return default;
    defer v.unref();
...
fn parseNode(item: *Item, value: *glib.Variant, parent: i32, depth: usize) !void {
    ...
    const label = db.string(props, "label", "s");
    node.label = @import("notification_policy.zig").sanitize(256, label.slice());
    const kind = db.string(props, "type", "s");
    node.separator = std.mem.eql(u8, kind.slice(), "separator");
    const display = db.string(props, "children-display", "s");
    node.submenu = std.mem.eql(u8, display.slice(), "submenu");
    const toggle = db.string(props, "toggle-type", "s");
    node.toggle = toggle.len != 0;
    if (db.lookup(props, "toggle-state", "i")) |v| {
        defer v.unref();
        node.checked = v.getInt32() == 1;
    }
    item.nodes[item.node_count] = node;
    item.node_count += 1;
    const children = value.getChildValue(2);
    defer children.unref();
    if (children.nChildren() > 128) return error.InvalidMenu;
    for (0..children.nChildren()) |i| {
        const wrapped = children.getChildValue(i);
        defer wrapped.unref();
        const child = wrapped.getVariant();
        defer child.unref();
        try parseNode(item, child, node.id, depth + 1);
    }
}
fn decodePixmap(images: *glib.Variant) ?*pixbuf.Pixbuf {
    var chosen: ?*glib.Variant = null;
    defer if (chosen) |v| v.unref();
    var best: i32 = std.math.maxInt(i32);
    for (0..@min(images.nChildren(), 16)) |i| {
        const v = images.getChildValue(i);
        const w = v.getChildValue(0);
        defer w.unref();
        const h = v.getChildValue(1);
        defer h.unref();
        const width = w.getInt32();
        const height = h.getInt32();
        ...
        if (distance < best) {
            if (chosen) |old| old.unref();
            chosen = v;
            best = @intCast(distance);
        } else v.unref();
    }
    ...
    const bytes = glib.Bytes.new(rgba.ptr, rgba.len);
    defer bytes.unref();
    return pixbuf.Pixbuf.newFromBytes(bytes, .rgb, 1, 8, width, height, width * 4);
}

test "ARGB network byte order, exact pixel length and empty pixmaps" {
    const t = std.testing;
    const byte_type = glib.VariantType.new("y");
    defer byte_type.free();
    const raw = glib.Variant.newFixedArray(byte_type, @ptrCast(&[_]u8{ 255, 1, 2, 3 }), 4, 1);
    const images = db.array("(iiay)", &.{db.tuple(&.{ glib.Variant.newInt32(1), glib.Variant.newInt32(1), raw })});
    _ = images.refSink();
    defer images.unref();
    const p = decodePixmap(images) orelse return error.DecodeFailed;
    defer p.unref();
    var n: c_uint = 0;
    const pixels = p.getPixelsWithLength(&n);
    try t.expectEqualSlices(u8, &.{ 1, 2, 3, 255 }, pixels[0..4]);
    const empty = db.array("(iiay)", &.{});
    _ = empty.refSink();
    defer empty.unref();
    try t.expect(decodePixmap(empty) == null);
}

test "DBusMenu rejects duplicate node IDs and excessive depth" {
    const t = std.testing;
    const child = db.tuple(&.{ glib.Variant.newInt32(1), db.array("{sv}", &.{}), db.array("v", &.{}) });
    const tree = db.tuple(&.{ glib.Variant.newInt32(1), db.array("{sv}", &.{}), db.array("v", &.{glib.Variant.newVariant(child)}) });
    _ = tree.refSink();
    defer tree.unref();
    var item: Item = .{};
    try t.expectError(error.InvalidMenu, parseNode(&item, tree, -1, 0));
    item = .{};
    try t.expectError(error.InvalidMenu, parseNode(&item, tree, -1, 9));
}
