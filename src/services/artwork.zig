...
pub const Art = struct {
    app: *gio.Application = undefined, context: *anyopaque = undefined, changed: *const fn (*anyopaque) void = undefined,
    uri: Text(512) = .{}, epoch: u64 = 0, job: ?*Job = null, image: ?*pixbuf.Pixbuf = null,
    pub fn want(self: *Art, uri: []const u8) void {
        if (std.mem.eql(u8, self.uri.slice(), uri)) return;
        self.uri.set(uri); self.epoch += 1;
        if (self.image) |image| image.unref(); self.image = null;
    ...
    fn launch(self: *Art) void {
        ...
        self.job = job; self.app.hold();
        const task = gio.Task.new(null, job.cancel, completed, job);
        task.setCheckCancellable(0); _ = task.setReturnOnCancel(0); task.setTaskData(job, null); task.runInThread(work); task.unref();
    }
    ...
    fn load(job: *Job) ?*pixbuf.Pixbuf {
        const file = gio.File.newForUri(job.uri.z()); defer file.unref();
        const path = file.getPath() orelse return null; defer glib.free(path);
        ...
        if (fd < 0) return null; defer _ = std.c.close(fd);
        ...
        const bytes = a.alloc(u8, limit + 1) catch return null; defer a.free(bytes);
        var used: usize = 0;
        while (used < bytes.len) {
            if (job.cancel.isCancelled() != 0) return null;
            const n = std.c.read(fd, bytes[used..].ptr, @min(65536, bytes.len - used));
            if (n < 0) return null; if (n == 0) break; used += @intCast(n);
        }
        if (used > limit or !dimensions(bytes[0..used])) return null;
        const storage = glib.Bytes.new(bytes.ptr, used); defer storage.unref();
        const stream = gio.MemoryInputStream.newFromBytes(storage); defer stream.unref();
    ...
    fn completed(_: ?*object.Object, result: *gio.AsyncResult, data: ?*anyopaque) callconv(.c) void {
        const job: *Job = @ptrCast(@alignCast(data.?)); const self = job.art;
        ...
        job.cancel.unref(); a.destroy(job);
        if (current) { self.image = image; self.changed(self.context); }
        else { if (image) |p| p.unref(); self.launch(); }
    }
};
fn valid(w: u32, h: u32) bool { return w > 0 and h > 0 and w <= 4096 and h <= 4096 and @as(u64, w) * h <= 8 * 1024 * 1024; }
pub fn dimensions(bytes: []const u8) bool {
    ...
    while (i + 4 <= bytes.len) {
        ...
        const marker = bytes[i + 1]; if (marker == 0xff) { i += 1; continue; }
