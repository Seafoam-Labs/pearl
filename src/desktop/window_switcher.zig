//! Workspace scope and eligibility shared by the bar and command controller.
...
pub fn count(model: *const Model, output: []const u8) usize {
    const workspace = model.activeWorkspace(output) orelse return 0;
    var total: usize = 0;
    var it = model.entities.valueIterator();
    while (it.next()) |entity| if (entity.* == .window and eligible(entity.window, output, workspace.id)) {
        total += 1;
    };
    return total;
}
pub fn action(model: *const Model, output: []const u8, direction: Direction, reduced_motion: bool) !commands.Action {
    const workspace = model.activeWorkspace(output) orelse return error.Unavailable;
    const focus = try model.focus(null);
    const fields: commands.Switcher = .{ .output = output, .workspace = workspace.id, .seat = focus.seat.id, .reduced_motion = reduced_motion };
