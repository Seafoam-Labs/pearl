...
pub const View = struct {
    ...
    rows: [64]Card = undefined,
    count: usize = 0,
    display_limit: usize = 3,
    dnd: ?*gtk.Button = null,
    ...
    pub fn create(host: *gtk.Box, notifications: *service.Notifications, toast: bool) !*View {
        ...
        if (!toast) {
            ...
            const dnd = gtk.Button.newWithLabel("");
            self.dnd = dnd;
            _ = gtk.Button.signals.clicked.connect(dnd, *View, toggleDnd, self, .{});
            controls.append(dnd.as(gtk.Widget));
            const clear = gtk.Button.newWithLabel(tr("Clear history", "Verlauf löschen"));
            _ = gtk.Button.signals.clicked.connect(clear, *View, clearHistory, self, .{});
            controls.append(clear.as(gtk.Widget));
            host.append(controls.as(gtk.Widget));
            self.empty = w.label("", "pearl-secondary");
            host.append(self.empty.?.as(gtk.Widget));
        ...
        for (self.rows[0..self.count]) |*row| {
            const box = w.card();
            box.setSpacing(8);
            box.as(gtk.Widget).addCssClass("pearl-notification-card");
            const header = w.label("", "pearl-secondary");
            const title = w.label("", "pearl-card-title");
            const body = w.label("", null);
            title.setLines(2);
            title.setEllipsize(.end);
            body.setLines(if (toast) 3 else 6);
            body.setEllipsize(.end);
            body.setMaxWidthChars(48);
            const heading = w.row(8);
            const icon = w.icon("pearl-notifications-symbolic");
            heading.append(icon.as(gtk.Widget));
            heading.append(header.as(gtk.Widget));
    ...
    pub fn update(self: *View) void {
        ...
        var records: [64]*service.policy.Record = undefined;
        var count: usize = 0;
        for (&model.records) |*r| if (r.id != 0 and (!self.toast or r.toast_until > glib.getMonotonicTime()) and !model.locked) {
            records[count] = r;
            count += 1;
        };
        ...
        if (!self.toast) {
            var grouped: [64]*service.policy.Record = undefined;
            var used: [64]bool = @splat(false);
            var n: usize = 0;
            for (records[0..count], 0..) |r, i| {
                if (used[i]) continue;
                for (records[0..count], 0..) |other, j| if (!used[j] and std.mem.eql(u8, r.app.slice(), other.app.slice())) {
                    grouped[n] = other;
                    n += 1;
                    used[j] = true;
                };
        ...
        for (self.rows[0..self.count], 0..) |*row, i| {
            const visible = i < count and (!self.toast or i < self.display_limit);
            row.box.as(gtk.Widget).setVisible(@intFromBool(visible));
            if (!visible) continue;
            const r = records[i];
            row.id = r.id;
            ...
            for (&row.actions, 0..) |*action, j| {
                action.id = r.id;
                if (j < r.action_count) {
                    action.key = r.actions[j];
                    action.button.setLabel(action.key.label.z());
                }
                const show = r.active and (j == 8 or (j < r.action_count and (!self.toast or j < 2)));
                action.button.as(gtk.Widget).getParent().?.setVisible(@intFromBool(show));
                action.button.as(gtk.Widget).setVisible(@intFromBool(show));
    ...
    fn clearHistory(_: *gtk.Button, self: *View) callconv(.c) void {
        self.service.model.clearHistory();
        self.service.update();
    }
};
