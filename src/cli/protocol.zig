...
pub fn parse(a: std.mem.Allocator, bytes: []const u8) !Request {
    ...
    while (it.next()) |field| {
        ...
        const allowed = switch (r.op) {
            .preferences_status, .preferences_reload, .status, .popup_hide, .launcher_hide, .quit => &[_][]const u8{},
