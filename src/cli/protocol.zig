...
pub fn parse(a: std.mem.Allocator, bytes: []const u8) !Request {
    ...
    while (it.next()) |field| {
        ...
        const allowed = switch (r.op) {
            ...
            .running_apps_show, .notifications_toggle, .media_toggle, .tray_toggle, .popup_show, .popup_toggle, .launcher_show, .launcher_toggle, .calendar_toggle, .overview_toggle, .layout_get => &[_][]const u8{"output"},
