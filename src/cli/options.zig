...
pub const usage =
    ...
    \\       pearlctl wm action --text ACTION_JSON
    \\       pearlctl capture windows [--offset N]
    \\       pearlctl capture window --text TOPLEVEL_ID
    ...
    \\       pearlctl plugins list [--offset N]
    \\       pearlctl plugins inspect|reload --path ID
    \\       pearlctl plugins enable --path ID --text PACKAGE_SHA256 --revision N
    \\       pearlctl plugins disable --path ID --revision N
...
test "running applications keyboard entry accepts only output selection" {
    const t = std.testing;
    const request = try parse(&.{ "running-apps", "show", "--output", "opaque-output" });
    try t.expectEqual(protocol.Op.running_apps_show, request.request.op);
    try t.expectEqualStrings("opaque-output", request.request.output.?);
    try t.expectError(error.Usage, parse(&.{ "running-apps", "show", "--page", "sound" }));
}
