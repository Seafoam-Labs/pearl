...
pub const Window = struct {
    ...
    pub fn style(self: *Window, snapshot: appearance.Snapshot) !void {
        ...
        const sheet = try std.fmt.allocPrintSentinel(a, "{s}\n{s}\n{s}", .{ if (snapshot.mode == .gtk) @embedFile("settings_native") else material, app_style, @embedFile("settings_layout") }, 0);
        defer a.free(sheet);
