...
pub const Window = struct {
    ...
    pub fn aqueousChanged(self: *Window) void {
        ...
        if (self.aqueous_view) |view| {
            if (view.rules_view) |rules| rules.visible = self.target.page == .aqueous;
    ...
    pub fn probe(self: *Window, alloc: std.mem.Allocator) ![]const u8 {
        ...
        if (self.target.page == .aqueous) if (self.aqueous_view) |view| {
            ...
            if (view.rules_view) |rules| for (rules.controls.items) |control| if (control.widget.getMapped() != 0) {
                try controls.append(alloc, .{ .field = control.id, .focused = if (focus) |widget| widget == control.widget else false, .enabled = control.widget.isSensitive() != 0, .bounds = self.bounds(control.widget) });
            };
