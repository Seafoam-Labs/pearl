...
pub const PluginRefresh = struct { view: []const u8, operation: []const u8 };
pub const PluginInfo = struct { available: bool = true, sources: []const @import("../plugins/model.zig").PackageSource = &.{}, source: []const u8 = "", path: []const u8 = "", shadowed: usize = 0, installed: bool = true, id: []const u8, name: []const u8, version: []const u8, digest: []const u8, capabilities: @import("../plugins/model.zig").Grants, settings: []const @import("../plugins/model.zig").Schema, status: []const u8, input_activity: []const u8 = "unsupported", error_code: ?[]const u8 = null };
pub const PluginPage = struct { refresh_available: bool = false, pending: bool = false, discovery_revision: []const u8 = "0", requested: []const u8 = "0", completed: []const u8 = "0", monitoring: []const u8 = "", summary: []const u8, plugins: []const PluginInfo = &.{}, next_offset: ?[]const u8 = null };
pub const Page = struct { notification_header: ?Row = null, bar_widgets: ?[]const @import("bar_model.zig").Plugin = null, refresh_available: bool = false, discovery_revision: p.Number = p.num(0), requested: p.Number = p.num(0), completed: p.Number = p.num(0), monitoring: []const u8 = "", plugins: []const PluginInfo = &.{}, summary: []const u8, pending: bool = false, truncated: bool = false, rows: []const Row, offset: p.Number, next_offset: ?p.Number = null, prompt: ?Prompt = null };
