...
pub const Page = struct { refresh_available: bool = false, discovery_revision: p.Number = p.num(0), requested: p.Number = p.num(0), completed: p.Number = p.num(0), monitoring: []const u8 = "", plugins: []const PluginInfo = &.{}, summary: []const u8, pending: bool = false, truncated: bool = false, rows: []const Row, offset: p.Number, next_offset: ?p.Number = null, prompt: ?Prompt = null };
