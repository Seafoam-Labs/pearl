...
pub const Request = struct {
    ...
    session: []const u8,
    display: []const u8,
    op: enum { hello, ping, @"page.enter", @"page.leave", @"page.get", @"document.get", @"document.read", @"document.begin", @"document.write", @"document.finish", @"document.cancel", @"draft.discard", @"draft.merge", @"draft.validate", @"draft.apply", @"operation.get", @"frontend.close", @"list.get", @"audio.set", @"brightness.set", @"profile.set", @"network.action", @"network.editor", @"bluetooth.action", @"prompt.answer", @"notifications.action", @"lifecycle.action", @"power.action", @"media.action", @"layout.get", @"layout.set", @"aqueous.get", @"aqueous.action" },
    epoch: ?[]const u8 = null,
